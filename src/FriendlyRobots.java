...
public class FriendlyRobots {
	...
	public FriendlyRobots() {
		...
		setupRobot(robot1, Color.magenta);
		setupRobot(robot2, Color.CYAN);
		// 4. Move robots back to the top-right and bottom-left corners
		for (int m = 0; m <50; m++){
		robot1.setX(xPosLeft);
		robot1.setY(yPosBottom);
		...
		for (int w = 0; w < 200; w++) {
			...
			seekX(robot2, robot1);
			seekY(robot2, robot1);
			// 7. Use the if statement to check whether robot friends have found
			// each other
			if (foundFriend(robot1, robot2) == true) {
				// 8. if the robots have found eachother make them sparkle and
				// dance.
				robot1.sparkle();
				robot2.sparkle();
				// 9. print out the number of times the robots had to move
				// before the found eachother.
